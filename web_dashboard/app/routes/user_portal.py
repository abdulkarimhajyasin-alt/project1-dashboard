...
def serialize_mining_status(status: dict, completed_cycle=None) -> dict:
    ...
    if completed_cycle is not None:
        ...
        return {
            ...
            "settled_user_profits": decimal_display(status.get("settled_user_profits", status.get("current_total_balance")), 8),
            "live_cycle_income": "0.00000000",
            "dashboard_earnings_total": decimal_display(status.get("dashboard_earnings_total", status.get("current_total_balance")), 8),
            "live_total_earnings": decimal_display(status.get("live_total_earnings", status.get("current_total_balance")), 8),
    ...
    return {
        ...
        "current_total_balance": decimal_display(status.get("current_total_balance"), 8),
        "settled_user_profits": decimal_display(status.get("settled_user_profits", status.get("current_total_balance")), 8),
        "live_cycle_income": decimal_display(status.get("live_cycle_income", status.get("live_earned_income")), 8),
        "dashboard_earnings_total": decimal_display(status.get("dashboard_earnings_total", status.get("live_available_yield")), 8),
        "live_total_earnings": decimal_display(status.get("live_total_earnings", status.get("live_available_yield")), 8),
